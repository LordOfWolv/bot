...
def detect_bird(image, model_path, labels_path):
...
async def check(ctx):
    ...
    if ctx.message.attachments:
        ...
        for i in ctx.message.attachments:
            ...
            await ctx.send(detect_bird(f'./{file_name}', 'keras_model.h5', 'labels.txt'))
            await ctx.send(f'Hi! I am a bot {bot.user}!')
    else:
...
bot.run('MTE1MjkwODQyMzIzNTY0OTYwNw.GgxfjA.R85P9JOM32JNsnvmDYi_kDaNF4vVWEVYMEtJc8')